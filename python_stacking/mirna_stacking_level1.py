...
pathways = glob(path_name + "Communities/*.txt")
labels = pd.read_csv(path_name + "Communities/labels",index_col = 0,sep=" ")
...
for i, p in enumerate(pathways):
	...
	for j, (train, test) in enumerate(StratifiedKFold(y,outer_folds,True)):
		...
		pd.DataFrame(gs.grid_scores_).to_csv(path_name + 'Communities_res/' + p_name + '_fold{}_grid_scores.txt'.format(j))
	...
	np.savetxt(path_name + 'Communities_res/' + p_name + '_conf_mat.txt', mean_cm)
	test_avg_scores[p_name] = dict(avg_acc=np.mean(test_scores[p_name]),best_params = gs.best_params_)

pd.DataFrame(test_avg_scores).T.to_csv(path_name + 'Communities_res/test_avg_accuracy.txt')
pd.DataFrame(test_scores).T.to_csv(path_name+ 'Communities_res/test_scores.txt')
pd.DataFrame(test_predictions, index=labels.index).to_csv(path_name+ 'Communities_res/level1_features.txt')
