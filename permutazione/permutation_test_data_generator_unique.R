...
evaluate_perm_test_results = function(real_accuracies, perm_test_accuracies,n_perm_test = n_perm_test, perm_dimensions, all_dimensions){
  percs = c()
  for(i in 1:length(real_accuracies)){
    path_dimension <- all_dimensions[i]
    perm_path_idx <- which(perm_dimensions == path_dimension)
    if (length(perm_path_idx) != 1) {
      stop("There should be only one permutation pathway with this dimension. Check your dataset!")
    }
    percs = c(percs,sum(real_accuracies[i] > perm_test_accuracies[perm_path_idx,]) / n_perm_test)
  }
  return(percs)
}
...
dimensions = get_pathway_dimensions("allRed")
dimensions <- sort(unique(dimensions))
